...
class YapealQueryBuilder implements Countable {
   ...
   * @param array $defaults Optional list of column defaults.
   *
   ...
   */
  public function __construct($tableName, $dsn, array $defaults = array()) {
  ...
  public function addRow($row) {
    ...
    $missing = array_diff(array_keys($this->colTypes), array_keys($data));
    if (count($missing)) {
      $mess = 'Missing required fields (' . implode(', ', $missing);
      $mess .= ') found in row while making upsert for ' . $this->tableName;
      trigger_error($mess, E_USER_WARNING);
      return FALSE;
    };
    ...
    $extras = array_diff(array_keys($data), array_keys($this->colTypes));
    if (count($extras)) {
  ...
  /**
   * Finishes building query and store it to database table it also clears rows.
   *
  ...
  public function store() {
    ...
    $sql .= implode(',', $updates);
    // Query is now complete should clear rows so another group(s) can be
    // processed if needed.
    $this->rows = array();
    // Use a transaction for larger upserts to make them faster.
    if ($cnt > 10) {
      $this->con->StartTrans();
